...
- Provider rate-limit and server errors retry with exponential backoff up to three attempts. Any other failure surfaces to the host as a plain message with a retry button. Per-call timeout is two minutes and the whole job times out at fifteen minutes.
